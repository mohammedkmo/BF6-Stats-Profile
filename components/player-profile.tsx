...
export function PlayerProfile({ stats, coverImageUrl }: PlayerProfileProps) {
  ...
  const shotAccuracyStat = combatStats.find(stat => stat.id === 'shot_accuracy') || highlightedStat?.id === 'shot_accuracy' ? highlightedStat : null;
  const combatStatsWithoutAccuracy = combatStats.filter(stat => stat.id !== 'shot_accuracy');
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      {/* Header Section */}
      <div className={`relative  ${coverImageUrl ? 'bg-zinc-950' : 'bg-zinc-900/50'}`}>
        <div className={`container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 ${coverImageUrl ? 'pt-8 pb-8' : 'py-8'}`}>
          {/* Changed flex from sm:flex-row to always flex-col on mobile, flex-row only on sm+, with avatar centered via justify-center */}
          <div className={`flex flex-col sm:flex-row sm:items-center gap-4 ${coverImageUrl ? 'sm:-mt-20' : ''} items-center sm:items-center`}>
            <Avatar className={`relative z-10 rounded-3xl h-16 w-16 border-2 border-zinc-700/50 ring-2 ring-zinc-700/20 sm:h-20 sm:w-20`} 
            ...
            <div className={`text-center md:text-start ${coverImageUrl ? 'sm:mt-2' : ''} w-full sm:w-auto`}>
              <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">
                {playerDisplayName}
              </h1>
              <p className="text-sm font-medium text-zinc-400">@{stats.playerTag}</p>
              
              <div className={`mt-4 flex flex-wrap items-center gap-2 justify-center sm:justify-start`}>
                <Badge variant="outline" className="border-red-500/30 bg-red-500/10 px-2.5 py-0.5 text-xs font-semibold text-red-400">
                  {gameMode.name}
                </Badge>
                <Badge variant="outline" className="border-zinc-700 bg-zinc-800/50 px-2.5 py-0.5 text-xs font-semibold text-zinc-300">
                  {playerRole.name}
                </Badge>
              </div>
            </div>
      ...
      <div className="container mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {(combatStats.length > 0 || shotAccuracyStat) && (
          <div className="mb-12">
            ...
            {highlightedStat && highlightedStat.id !== 'shot_accuracy' && (
              <div className="mt-6 animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${combatStatsWithoutAccuracy.length * 50}ms` }}>
        ...
        {additionalStatsGroups.length > 0 && additionalStatsGroups.map((group, groupIndex) => (
